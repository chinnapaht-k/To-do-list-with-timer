...
listContainer.addEventListener("click",function(e){
    ...
    else if (e.target.tagName === "SPAN" && e.target.parentElement.classList =='checked'){
...
const buttonSound = new Audio("/static/sound/button-sound.mp3");
...
function startTimer() { 
    ...
    interval = setInterval(function() { //excute callback funtion every 1000 mili seconds
      ...
      if (total <= 0 && timer.mode=="pomodoro"){
        fetch('/addtimernum', {
          method: "POST",
          body: JSON.stringify({timer: timer.pomodoro}),
        });  
        clearInterval(interval);
        switchSession(timer)
        
      }
      else if (total <= 0) { //cause setInterval() to be cancelled and countdown end
        clearInterval(interval);
        switchSession(timer);
  }
    }, 1000);
...
function setNewTime(timer){
  updatePomodoro = parseInt(document.getElementById('pomodoro-time').value)
  updateShortBreak = parseInt(document.getElementById('shortbreak-time').value)
  updateLongBreak = parseInt(document.getElementById('longbreak-time').value)
  if (updatePomodoro >= 300 || updateShortBreak >= 300 || updateLongBreak >= 300){
    alert("Your timer should not be higher than 300 minutes")
  }
  else if (updatePomodoro <= 0 || updateShortBreak <= 0 || updateLongBreak <=0){
    alert("Your timer can not be 0 or less than 0")
  }
  else {
    alert("in?")
    alert(updatePomodoro)
    timer["pomodoro"] = updatePomodoro
    alert("asd")
    timer["longBreak"] = updateLongBreak
    timer["shortBreak"] = updateShortBreak
    alert("Your input have been updated")
  }
}
